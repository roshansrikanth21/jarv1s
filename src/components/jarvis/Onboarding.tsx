...
export function Onboarding({ onComplete }: { onComplete: (name: string) => void }) {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (d.type === "model_pull") {
          ...
          const err =
            d.status === "error" ? String(d.error ?? "Download blocked for this device.") : "";
          setPulls((p) => ({
            ...p,
  ...
  const installedRunnable = (models?.installed ?? []).filter(
    (m) => m.tools && m.runnable !== false,
  );
  const hasBrain =
    installedRunnable.length > 0 || Boolean(keyStatus?.groq) || Boolean(keyStatus?.anthropic);

  const finish = async () => {
    setSaving(true);
    if (hasElectron && groqKey.trim()) {
      try {
        setKeyStatus(await window.electronAPI!.setApiKeys!({ GROQ_API_KEY: groqKey.trim() }));
      } catch (e) {
        setKeyErr(e instanceof Error ? e.message : "Couldn't save key.");
        setSaving(false);
        return;
      }
    }
    ...
    try {
      localStorage.setItem("jarvis_user_name", name.trim());
    } catch {
      /* ignore */
    }
    ...
    const bestRec =
      (latest?.recommended ?? []).find((r) => r.best && r.installed) ??
      (latest?.recommended ?? []).find((r) => r.installed);
    const pinTag =
      bestRec?.tag ?? runnableLatest.sort((a, b) => (b.gb ?? 0) - (a.gb ?? 0))[0]?.name;
    const pinAllowed =
      pinTag &&
    ...
    <div className="onb-root">
      ...
      >
        <div className="onb-stepper">
          {["You", "Local brain", hasElectron ? "Cloud key" : null]
            .filter(Boolean)
            .map((label, i) => (
              <div
                key={i}
        ...
        <AnimatePresence mode="wait">
          {step === 0 && (
            ...
            >
              <div className="onb-reactor">
                <ArcReactor active size="sm" />
              </div>
              <h1 className="onb-title">I'm JARVIS.</h1>
              <p className="onb-sub">
                Your personal AI — I run on your machine and scale the model to your hardware. Let's
                get set up. What should I call you?
              </p>
              <input
                className="onb-input"
                value={name}
          ...
          {step === 1 && (
            ...
            >
              ...
              <p className="onb-sub">
                ...
                The <b style={{ color: AMBER }}>BEST</b> pick balances quality and speed on your
                hardware.
              </p>

              <div className="onb-rig">
                <span>
                  <Cpu size={12} /> {device?.cpu?.name ? shortCpu(device.cpu.name) : "CPU"}
              ...
              {models?.budget?.instant_tight && (
                <div className="onb-note onb-note--warn">
                  RAM is tight right now ({models.budget.ram_available_gb}GB free) — close a few
                  apps before downloading a large model.
                </div>
              )}

              {models && models.budget && !models.budget.local_viable && (
                <div className="onb-note onb-note--warn">
                  Not enough RAM for a local model on this PC (~{models.budget.budget_gb}GB budget
                  after system reserve). Skip this step and add a free{" "}
                  <b style={{ color: AMBER }}>Groq</b> key next — JARVIS will run in the cloud.
                </div>
              )}
              ...
              {models && !models.ollama && (
                <div className="onb-note onb-note--warn">
                  ...
                  <button
                    className="onb-link"
                    onClick={() => openExt("https://ollama.com/download")}
                  >
                    ollama.com ↗
                  </button>{" "}
                  and reopen JARVIS — or just add a cloud key on the next step.
                </div>
              )}

              {installedRunnable.length > 0 && (
                <div className="onb-note onb-note--ok">
                  <Check size={12} /> You already have{" "}
                  {installedRunnable.map((m) => m.name.split(":")[0]).join(", ")} — you're ready.
                </div>
              )}

              <div className="onb-models">
                {(models?.recommended ?? []).length === 0 && models?.budget?.local_viable === false
                  ? null
                  : (models?.recommended ?? []).map((rec) => {
                      const p = pulls[rec.tag];
                      const installing =
                        p && p.status !== "success" && p.status !== "done" && p.pct < 100;
                      return (
                        <div key={rec.tag} className={`onb-model ${rec.best ? "is-best" : ""}`}>
                          ...
                          <div className="onb-model-meta">
                            {rec.params} · {rec.gb}GB disk
                            {rec.needs_gb ? ` · ~${rec.needs_gb}GB RAM` : ""}
                            {rec.ctx ? ` · ${rec.ctx} ctx` : ""}
                          </div>
                          {rec.best_for && <div className="onb-model-for">{rec.best_for}</div>}
                          {rec.limits && (
                            <div className="onb-model-lim">Trade-off: {rec.limits}</div>
                          )}
                          <div className="onb-model-foot">
                            {rec.installed ? (
                              <span className="onb-installed">
                                <Check size={12} /> Installed
                              </span>
                            ) : installing ? (
                              <div className="onb-prog">
                                <div className="onb-prog-bar">
                                  <div className="onb-prog-fill" style={{ width: `${p.pct}%` }} />
                                </div>
              ...
              <div className="onb-actions">
                <button className="onb-btn onb-btn--text" onClick={() => setStep(0)}>
                  Back
                </button>
          ...
          {step === 2 && (
            ...
            >
              <div className="onb-reactor">
                <Sparkles size={26} color={AMBER} />
              </div>
              <h1 className="onb-title">Want faster, smarter answers?</h1>
              <p className="onb-sub">
                Add a free <b style={{ color: AMBER }}>Groq</b> key and I'll route hard questions to
                the cloud — replies in a second or two instead of tens of seconds. Stored encrypted
                in your OS keychain, never shared. Totally optional.
              </p>
              <div className="onb-key">
                <div className="onb-key-head">
                  <span>
                    <KeyRound size={12} /> Groq API key{" "}
                    {keyStatus?.groq && <span style={{ color: GREEN }}>· set</span>}
                  </span>
                  <button
                    className="onb-link"
                    onClick={() => openExt("https://console.groq.com/keys")}
                  >
                    Get a free key ↗
                  </button>
                </div>
                <input
                  ...
                  spellCheck={false}
                  value={groqKey}
                  placeholder={keyStatus?.groq ? "•••••••• (leave blank to keep)" : "gsk_…"}
                  onChange={(e) => setGroqKey(e.target.value)}
                />
                {keyErr && <div className="onb-note onb-note--warn">{keyErr}</div>}
              </div>
              <div className="onb-actions">
                <button className="onb-btn onb-btn--text" onClick={() => setStep(1)}>
                  Back
                </button>

...
export function SettingsPanel({
  ...
  return (
    <div style={c.overlay} className="no-drag" onMouseDown={onClose}>
      <div style={c.card} onMouseDown={e => e.stopPropagation()} role="dialog" aria-label="Settings">
        <div style={c.header}>
          <span style={c.title}>Settings</span>
          <button style={c.close} onClick={onClose} title="Close (Esc)" aria-label="Close">✕</button>
        </div>
        ...
        <div style={c.body}>
          ...
          {!hasElectron ? (
            <p style={c.note}>
              Secure key storage runs in the desktop app. In the browser/dev, set keys in your <code>.env</code>.
            </p>
            ...
            <p style={c.note}>Encrypted with your OS keychain (Windows DPAPI). Never written in plaintext.</p>
          )}

          <KeyRow
            c={c} label="Groq" hint="free & fast — recommended"
            configured={Boolean(keyStatus?.groq)} disabled={!hasElectron}
            value={groqKey} onChange={setGroqKey} placeholder="gsk_…"
            onGet={() => openLink("https://console.groq.com/keys")}
            inputRef={firstFieldRef}
          />
          <KeyRow
            c={c} label="Anthropic" hint="optional — Claude for the hardest asks"
          ...
          {s && (
            <>
              ...
              <select style={c.input} value={s.voice} onChange={e => patch("voice", e.target.value)}>
              ...
              <Toggle
                c={c} label="Always-on listening"
                sub="Auto-start the mic on launch (still only acts on “Jarvis …”)."
                on={s.always_listen} onToggle={() => patch("always_listen", !s.always_listen)}
              />
              <Toggle
                c={c} label="Remember overheard speech"
                sub="Transcribe & keep ambient audio near the mic. Off = only your commands."
                on={s.store_overheard} onToggle={() => patch("store_overheard", !s.store_overheard)}
              />
            </>
          )}
        ...
        <div style={c.footer}>
          <button style={c.ghostBtn} onClick={onClose}>Cancel</button>
...
function KeyRow({
  c, label, hint, configured, disabled, value, onChange, placeholder, onGet, inputRef,
}: {
  c: Pal; label: string; hint: string; configured: boolean; disabled?: boolean;
  value: string; onChange: (v: string) => void; placeholder: string; onGet: () => void;
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        ...
        />
        <button style={c.getBtn} onClick={onGet} title={`Get a ${label} key`}>Get key ↗</button>
      </div>
    </div>
  );
}
...
function Toggle({
  c, label, sub, on, onToggle,
}: { c: Pal; label: string; sub: string; on: boolean; onToggle: () => void }) {
  return (
...
function palette(accent: string) {
  const input: CSSProperties = {
    ...
    borderRadius: 9, padding: "9px 11px", color: "#eef2f5",
  ...
  return {
    ...
    overlay: {
      ...
      background: "rgba(4,6,9,0.62)", backdropFilter: "blur(6px)",
      fontFamily: "ui-sans-serif, system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif",
    } as CSSProperties,
    card: {
      width: "min(460px, 92vw)", maxHeight: "88vh", display: "flex", flexDirection: "column",
    ...
    header: {
      ...
      padding: "15px 18px", borderBottom: "1px solid rgba(255,255,255,0.07)",
    } as CSSProperties,
    title: { fontSize: 15, fontWeight: 600, letterSpacing: "0.02em", color: "#f2f5f8" } as CSSProperties,
    close: {
      background: "transparent", border: "none", color: "rgba(236,240,244,0.55)",
    ...
    sectionLabel: {
      ...
      color: hexA(accent, 0.85), margin: "16px 0 9px",
    } as CSSProperties,
    note: { fontSize: 11.5, color: "rgba(230,235,240,0.46)", margin: "0 0 8px", lineHeight: 1.5 } as CSSProperties,
    input,
    keyLabel: { fontSize: 13, fontWeight: 600, color: "#eef2f5" } as CSSProperties,
    pillOn: {
      fontSize: 10.5, fontWeight: 600, color: accent, background: hexA(accent, 0.14),
      border: `1px solid ${hexA(accent, 0.4)}`, borderRadius: 999, padding: "2px 8px",
    } as CSSProperties,
    pillOff: {
      fontSize: 10.5, color: "rgba(230,235,240,0.5)", background: "rgba(255,255,255,0.05)",
      border: "1px solid rgba(255,255,255,0.12)", borderRadius: 999, padding: "2px 8px",
    } as CSSProperties,
    getBtn: {
      flexShrink: 0, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)",
      borderRadius: 9, color: "rgba(236,240,244,0.8)", fontFamily: "inherit", fontSize: 12,
      padding: "0 12px", cursor: "pointer", whiteSpace: "nowrap",
    } as CSSProperties,
    segRow: { display: "flex", gap: 5, marginBottom: 4 } as CSSProperties,
    seg: {
      flex: 1, textTransform: "capitalize", background: "rgba(255,255,255,0.04)",
      border: "1px solid rgba(255,255,255,0.1)", borderRadius: 9, padding: "8px 0",
      color: "rgba(236,240,244,0.72)", fontFamily: "inherit", fontSize: 12.5, cursor: "pointer",
    } as CSSProperties,
    segOn: {
      background: hexA(accent, 0.16), border: `1px solid ${hexA(accent, 0.55)}`,
    ...
    switch: {
      flexShrink: 0, width: 34, height: 18, borderRadius: 999, padding: 2,
    ...
    ghostBtn: {
      background: "transparent", border: "1px solid rgba(255,255,255,0.14)", borderRadius: 9,
      color: "rgba(236,240,244,0.8)", fontFamily: "inherit", fontSize: 13, padding: "8px 16px", cursor: "pointer",
    } as CSSProperties,
    primaryBtn: {
      background: accent, border: "none", borderRadius: 9, color: "#0a0c10",
      fontFamily: "inherit", fontSize: 13, fontWeight: 700, padding: "8px 20px", cursor: "pointer",
    } as CSSProperties,
  };
}

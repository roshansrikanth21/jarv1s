...
function subject(s: OpsStep): string {
  ...
  if (s.action === "scope") return [a.action, a.target].filter(Boolean).join(" ");
  ...
  return (
    <div style={{ display: "flex", gap: 10, position: "relative" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, paddingBottom: 16 }}>
          ...
          <>
            <pre
              style={{
                margin: "7px 0 3px",
                padding: "9px 11px",
                background: "oklch(0.12 0.015 28 / 0.7)",
        ...
        {running && (
          <div
            style={{ fontSize: 10.5, color: "var(--c-muted)", marginTop: 6, fontStyle: "italic" }}
          >
...
export function OpsConsole({
  ...
  const opCount = trace.filter((s) =>
    ["recon", "pentest", "scope", "browse", "report"].includes(s.action),
  ).length;
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        display: "flex",
        flexDirection: "column",
        background: "oklch(0.09 0.013 28 / 0.97)",
    ...
    >
      ...
      <div
        style={{ fontSize: 9.5, color: "var(--c-muted)", padding: "7px 14px 0", lineHeight: 1.4 }}
      >
        Real tool executions, live. If JARVIS claims it did something and there's no step here, it
        didn't happen.
      </div>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "14px 14px 8px" }}>
        {trace.length === 0 && !running && (
          ...
          >
            ...
            <span style={{ color: accent }}>pentest</span> — every step it actually runs shows here,
            live.
          </div>
        )}
        {trace.map((s, i) => (
          <Step key={`${s.step}-${i}`} s={s} />
        ))}
        {showRunning && (
          <Step
            s={{

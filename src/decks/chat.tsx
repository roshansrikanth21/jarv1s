...
export default function ChatDeck() {
  const { connected, listening, speaking, lines, stream, send, toggleMic, addLine, sendAction, showReconnectHint } =
  ...
  useEffect(() => {
    const onPaste = (e: ClipboardEvent) => {
      ...
      if (files && files.length) { e.preventDefault(); onFiles(files); }
    };
  ...
  return (
    ...
    >
      ...
      {sidebarOpen && (
        <div style={{
          width: 248, flexShrink: 0, background: SIDEBAR_BG, display: "flex", flexDirection: "column",
          padding: "12px 10px", gap: 6, borderRight: `1px solid ${BORDER}22`,
        }}>
          <button
            ...
            style={{
              ...
              color: TEXT, fontFamily: "inherit", fontSize: 13, cursor: "pointer", textAlign: "left",
            }}
          >
            <PlusIcon /> New chat
          </button>
          <div style={{ marginTop: 14, fontSize: 11, color: TEXT_DIM, padding: "0 12px", letterSpacing: "0.02em" }}>
            This session
          </div>
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 2 }}>
            {userTurns.length === 0 && (
              <div style={{ fontSize: 12.5, color: TEXT_DIM, padding: "6px 12px" }}>No messages yet</div>
            )}
            {userTurns.map(l => (
              <div key={l.id} title={l.text} style={{
                padding: "8px 12px", borderRadius: 8, fontSize: 12.5, color: "#d5d5d5",
          ...
          <div style={{ fontSize: 11, color: TEXT_DIM, padding: "8px 12px", borderTop: `1px solid ${BORDER}44` }}>
            JARVIS · local
            <span style={{
              display: "inline-block", width: 7, height: 7, borderRadius: "50%", marginLeft: 8,
        ...
        <div className="drag" style={{
          ...
          <div className="no-drag" style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={() => setSidebarOpen(o => !o)} title="Toggle sidebar" style={{
              background: "transparent", border: "none", color: TEXT_DIM, cursor: "pointer",
          ...
          <div className="no-drag"><WindowControls accent="#10a37f" /></div>
        </div>

        {/* conversation */}
        <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "8px 16px 24px" }}>
          <div style={{ maxWidth: 768, margin: "0 auto", display: "flex", flexDirection: "column", gap: 22 }}>
            {lines.length <= 1 && !stream && (
              <div style={{ textAlign: "center", marginTop: "22vh", color: TEXT, fontSize: 26, fontWeight: 600 }}>
                What can I help with?
              </div>
            )}
            {lines.map(l => <Message key={l.id} role={l.role} text={l.text} />)}
          ...
          <div style={{ maxWidth: 768, margin: "0 auto" }}>
            {attachments.length > 0 && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
                {attachments.map(a => (
                  <div key={a.id} style={{
                    display: "flex", alignItems: "center", gap: 8, background: BUBBLE_BG,
                    border: `1px solid ${BORDER}`, borderRadius: 12, padding: "6px 10px", fontSize: 12.5,
                  }}>
                    {a.preview
                      ? <img src={a.preview} alt="" style={{ width: 34, height: 34, objectFit: "cover", borderRadius: 7 }} />
                      : <FileIcon />}
                    <div style={{ maxWidth: 180 }}>
                      <div style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{a.name}</div>
                      <div style={{ fontSize: 10.5, color: a.status === "error" ? "#e06c6c" : TEXT_DIM }}>
                        {a.status === "uploading" ? "reading…" : a.status === "error" ? (a.note || "failed") : a.kind}
                      </div>
                    </div>
                    <button onClick={() => setAttachments(p => p.filter(x => x.id !== a.id))} style={{
                      background: "transparent", border: "none", color: TEXT_DIM, cursor: "pointer", fontSize: 14, padding: 2,
                    }}>×</button>
                  </div>
                ))}
              </div>
            ...
            <div style={{
              ...
              <textarea
                ...
                style={{
                  ...
                  color: TEXT, fontFamily: "inherit", fontSize: 15, lineHeight: 1.5, padding: "6px 0",
                  maxHeight: 200,
                }}
              />
              <button onClick={toggleMic} title={listening ? "Stop listening" : "Voice input"} style={{
                ...iconBtn, color: listening ? "#10a37f" : TEXT_DIM,
              }}>
                <MicIcon />
              </button>
              <button
                ...
                style={{
                  border: "none", cursor: busy ? "wait" : "pointer", borderRadius: "50%",
                  width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center",
                  background: input.trim() || attachments.length ? TEXT : "#4a4a4a",
                  color: MAIN_BG, flexShrink: 0,
                  opacity: busy ? 0.6 : 1,
                }}
              >
                <SendIcon />
            ...
            <div style={{ textAlign: "center", fontSize: 11, color: TEXT_DIM, marginTop: 8 }}>
              JARVIS runs on your machine. Drop images, PDFs, or code — it reads them before answering.
            </div>
          </div>
        </div>
      </div>
...
const iconBtn: React.CSSProperties = {
  background: "transparent", border: "none", color: TEXT_DIM, cursor: "pointer",
  padding: 7, borderRadius: 8, display: "flex", flexShrink: 0,
};
...
function Message({ role, text, streaming }: { role: Role; text: string; streaming?: boolean }) {
  ...
  if (role === "user") {
    return (
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <div style={{
          maxWidth: "70%", background: BUBBLE_BG, borderRadius: 20, padding: "10px 16px",
  ...
  return (
    <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
      ...
      <div style={{
        flex: 1, fontSize: 15, lineHeight: 1.7, whiteSpace: "pre-wrap", wordBreak: "break-word", paddingTop: 3,
      }}>
...
function PlusIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 5v14M5 12h14" /></svg>;
}
function MenuIcon() {
  return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M4 6h16M4 12h16M4 18h16" /></svg>;
}
function ClipIcon() {
  return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M21.4 11.05l-9.19 9.19a6 6 0 01-8.49-8.49l9.2-9.19a4 4 0 015.65 5.66l-9.2 9.19a2 2 0 01-2.82-2.83l8.49-8.48" /></svg>;
}
function MicIcon() {
  return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><rect x="9" y="2" width="6" height="12" rx="3" /><path d="M5 10v1a7 7 0 0014 0v-1M12 18v4" /></svg>;
}
function FileIcon() {
  return <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><path d="M14 2v6h6" /></svg>;
}
function SendIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7" /></svg>;
}
